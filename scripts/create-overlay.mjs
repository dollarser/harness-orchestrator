...
if (!input || !output) throw new Error('Usage: node scripts/create-overlay.mjs <config.json|--configure> <output.yml>');
if (resolve(input) === resolve(output)) throw new Error('Input and output must differ');
const config = input === '--configure' ? {} : parseConfig(JSON.parse(await readFile(input, 'utf8')));
